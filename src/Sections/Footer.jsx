        ...
        <footer className="max-container">
            <div className="flex justify-between items-start gap-20 flex-wrap max-lg:flex-col">
                <div className="flex flex-col items-start">
                    <a href="/">
                    <img src={footerLogo} alt="logo" height={46} width={150}/></a>
                    ...
                    <div className='flex items-center gap-5 mt-8'>
                        {socialMedia.map((icon, idx) => (<div key={idx} className='flex justify-center items-center w-12 h-12 bg-white rounded-full'>
                ...
                <div className='flex flex-1 justify-between gap-20 lg:gap-10 flex-wrap'>
                    {footerLinks.map((section) => (<div key={section}>
                        ...
                        <ul>
                            {section.links.map((link) => (<li key={link.name} className='mt-3 text-white-400 font-montserrat text-base leading-normal hover:text-slate-gray cursor-pointer'><a>{link.name}</a></li>))}
                        </ul>
                    </div>))}
                </div>
            </div>
